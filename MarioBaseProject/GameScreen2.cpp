...
GameScreen2::GameScreen2(SDL_Renderer* renderer) : GameScreen(renderer)
{
	SetUpLevel();
}
...
void GameScreen2::Update(float deltaTime, SDL_Event e)
{
...
bool GameScreen2::SetUpLevel()
{
	...
	mario = new CharacterMario(m_renderer, "Images/Characters/PlayerTest.png", Vector2D(1150, 400), m_level_map, FACING_RIGHT, MOVEMENTSPEED);
	luigi = new CharacterLuigi(m_renderer, "Images/Characters/CatTest.png", Vector2D(1150, 400), m_level_map, FACING_LEFT, MOVEMENTSPEED);
	m_end_door = new EndDoor(m_renderer, "Images/EndDoor.png", Vector2D(250, 650));

	//CreateKoopa(Vector2D(200, 100), FACING_RIGHT, KOOPA_SPEED);
	//CreateKoopa(Vector2D(400, 200), FACING_LEFT, KOOPA_SPEED);

	m_pow_block = new PowBlock(m_renderer, m_level_map, Vector2D(1575, 100));
	new_enemy_timer = 8;
	mario_score = 0;
	luigi_score = 0;
